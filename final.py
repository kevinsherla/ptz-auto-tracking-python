...
PANTILT_THRESHOLD = 150  # Adjusted threshold for smoother movement
DEAD_ZONE = 150  # Larger dead zone around the center to reduce unnecessary movements
COMMAND_DELAY = 0.5  # Increased delay for smoother response
PAN_SPEED = 8  # Increased pan speed for quicker adjustment
TILT_SPEED = 8  # Increased tilt speed for quicker adjustment
    ...
    if results.detections:
        for detection in results.detections:
            ...
            if abs(delta_x) > DEAD_ZONE or abs(delta_y) > DEAD_ZONE:
                ...
                avg_delta_x = (prev_face_center_x - CENTER_X + delta_x) // 2
                avg_delta_y = (prev_face_center_y - CENTER_Y + delta_y) // 2

                # Determine if the face is out of the center
                if abs(avg_delta_x) > PANTILT_THRESHOLD:
                    if avg_delta_x < 0:
                        command = f'left&{PAN_SPEED}&10'
                    else:
                        command = f'right&{PAN_SPEED}&10'

                if abs(avg_delta_y) > PANTILT_THRESHOLD:
                    if avg_delta_y < 0:
